...
private:
    double interaction_val;
    std::vector<Symbiont> sym;
    std::set<int> res_types;
    double points;
    int infection_rate;
    
public:
 Host(double _intval =0.0, std::vector<Symbiont> _sym = {}, std::set<int> _set = std::set<int>(), double _points = 0.0, int infection_rate=1) : interaction_val(_intval), sym(_sym), res_types(_set), points(_points) { ; }
    ...
    double GetIntVal() const { return interaction_val;}
    std::vector<Symbiont> GetSymbiont() { return sym;}
    std::set<int> GetResTypes() const { return res_types;}
    double GetPoints() { return points;}
    
    
    void SetIntVal(double _in) {interaction_val = _in;}
    void SetSymbiont(std::vector<Symbiont> _in) {sym = _in;}
    void SetResTypes(std::set<int> _in) {res_types = _in;}
    void SetPoints(double _in) {points = _in;}
    void AddPoints(double _in) {points += _in;}
    
    void GiveSymPoints(double _in, int index) {
        double distrib = _in;
        sym[index].AddPoints(distrib);
        
    }
    
    void ResetSymPoints(int index) {
        sym[index].SetPoints(0.0);
    }
    
    
    
    void SetSymIntVal (double _in, int index) {
        sym[index].SetIntVal(_in);
    ...
    void AddSym(Symbiont inSym) {
      //check the cap here
      //the host needs to know infection rate (field)
      if(NumSym() < infection_rate){
        sym.push_back(inSym);
      }
    }
    ...
    void DistribResources(int index, double resources, double synergy) {
        ...
        double symIntVal = sym[index].GetIntVal();
        ...
        if (hostIntVal >= 0 && symIntVal >= 0)  {
            ...
            this->GiveSymPoints(symPortion, index);
    ...
    void Process(emp::Random &random) {
      ...
      double split_resources = 100 / NumSym();
        //TODO: splits the resources evenly
        for(int i = 0; i < NumSym(); i++){
            DistribResources(i,split_resources, 5);
        }
    }    
};
...
std::string PrintHost(Host * org) {
    ...
    std::string formattedstring = temp.str();
    std::string out_val;
    std::vector<Symbiont> syms = org->GetSymbiont();
    for(int i = 0; i < org->NumSym(); i++){
        out_val += formattedstring + "/" + PrintSym(syms[i])+"\n";
    }
